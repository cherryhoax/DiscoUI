...
class DiscoLongListSelector extends DiscoPickerBox {
  ...
  constructor(title = 'SELECT SECTION', items = [], options = {}) {
    ...
    this.setAttribute('animation', 'slide-up');
  ...
  open() {
    ...
    this.show().then(() => {
      ...
      this._grid?.classList.add('shown-animation');
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          this._grid?.classList.remove('shown-animation');
        });
      });
    });
  ...
  _renderEntries() {
    ...
    this._entries.forEach((entry, index) => {
      ...
      tile.style.setProperty('--row-index', String(Math.floor(index / (customMode ? 1 : 4))));
